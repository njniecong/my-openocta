...
function renderFilterChips(
  ...
  const sessionsLabel = selectedSession
    ...
    : selectedSessions.length === 1
      ...
      : `${selectedSessions.length} ${t("usageSessionsCount")}`;
  ...
  return html`
    <div class="card sessions-card">
      <div class="sessions-card-header">
        <div class="card-title">${t("usageSessionsCard")}</div>
        <div class="sessions-card-count">
          ${sessions.length} ${t("usageShown")}${totalSessions !== sessions.length ? ` · ${totalSessions} ${t("usageTotalSessions")}` : ""}
        </div>
      </div>
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          <span>${isTokenMode ? formatTokens(avgValue) : formatCost(avgValue)} ${t("usageAvg")}</span>
          <span>${totalErrors} ${t("usageErrors").toLowerCase()}</span>
        </div>
        ...
        <label class="sessions-sort">
          ...
          >
            ...
            <option value="errors" ?selected=${sessionSort === "errors"}>${t("usageErrorsCol")}</option>
      ...
      ${
        sessionsTab === "recent"
          ? recentEntries.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usageNoRecentSessions")}</div>
              `
            : html`
          ...
          : sessions.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usageNoSessionsInRange")}</div>
              `
            : html`
        ...
        selectedCount > 1
          ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">${t("usageSelectedCount")} (${selectedCount})</div>
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageMessages")}</div>
        <div class="session-summary-value">${usage.messageCounts?.total ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.user ?? 0} ${t("usageUser").toLowerCase()} · ${usage.messageCounts?.assistant ?? 0} ${t("usageAssistant").toLowerCase()}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageToolCalls")}</div>
        <div class="session-summary-value">${usage.toolUsage?.totalCalls ?? 0}</div>
        <div class="session-summary-meta">${usage.toolUsage?.uniqueTools ?? 0} ${t("usageToolsLabel").toLowerCase()}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageErrors")}</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.toolResults ?? 0} ${t("usageToolResults")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageDuration")}</div>
    ...
    <div class="usage-insights-grid" style="margin-top: 12px;">
      ${renderInsightList(t("usageTopTools"), toolItems, t("usageNoToolCalls"))}
      ${renderInsightList(t("usageModelMix"), modelItems, t("usageNoModelData"))}
    </div>
  `;
}
      ...
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            usage
              ? html`
            <span><strong>${formatTokens(usage.totalTokens)}</strong> ${t("usageTokensUnit")}</span>
...
function renderTimeSeriesCompact(
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usageLoading")}</div>
      </div>
    `;
  }
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usageNoTimelineData")}</div>
      </div>
    `;
  }
  ...
  if (points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usageNoDataInRange")}</div>
      </div>
    `;
  }
  let cumTokens = 0,
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 13px;">${t("usageUsageOverTime")}</div>
      ...
      <svg viewBox="0 0 ${width} ${height + 15}" class="timeseries-svg" style="width: 100%; height: auto;">
        ...
        ${points.map((p, i) => {
          ...
          const tooltipLines = [
            ...
            `${formatTokens(val)} ${t("usageTokensUnit")}`,
          ];
          if (breakdownByType) {
      ...
      <div class="timeseries-summary">${points.length} ${t("usageMessagesCount")} · ${formatTokens(cumTokens)} ${t("usageTokensUnit")} · ${formatCost(cumCost)}</div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px;">${t("usageTokensByType")}</div>
                ...
                <div class="cost-breakdown-total">${t("usageTotalLabel")}: ${formatTokens(totalTypeTokens)}</div>
              </div>
            `
          : nothing
      }
  ...
  if (!contextWeight) {
    return html`
      <div class="context-details-panel">
        <div class="muted" style="padding: 20px; text-align: center">${t("usageNoContextData")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 13px;">${t("usageSystemPromptBreakdown")}</div>
        ${
          hasMore
            ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t("usageCollapseAll") : t("usageExpandAll")}
              </button>`
            : nothing
        }
      </div>
      <p class="context-weight-desc">${contextPct || t("usageBaseContextPerMessage")}</p>
      ...
      <div class="context-total">${t("usageTotalLabel")}: ~${formatTokens(totalContextTokens)}</div>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usageSkills")} (${skillsList.length})</div>
        ...
        ${
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usageToolsLabel")} (${toolsList.length})</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
                          <div class="context-breakdown-item">
        ...
        ${
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usageFiles")} (${filesList.length})</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usageConversation")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usageLoading")}</div>
      </div>
    `;
  }
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usageConversation")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usageNoMessages")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-logs-compact">
      ...
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("usageUser")}</option>
      ...
      <div class="session-logs-list">
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("usageNoMessagesMatchFilters")}</div>
              `
            : nothing
        }
      </div>
...
export function renderUsage(props: UsageProps) {
        ...
        <div class="row" style="justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
          <div style="flex: 1; min-width: 250px;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2px;">
              <div class="card-title" style="margin: 0;">${t("usageTokenUsage")}</div>
              <span style="
                display: inline-flex;
                align-items: center;
                gap: 6px;
  ...
  return html`
    ...
    <section class="usage-page-header">
      ...
      <div class="usage-page-subtitle">${t("usagePageSubtitle")}</div>
    </section>
    ...
    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      <div class="usage-header-row">
        ...
        <div class="usage-header-metrics">
            ...
            displayTotals
              ? html`
                <span class="usage-metric-badge">
                  <strong>${formatTokens(displayTotals.totalTokens)}</strong> ${t("usageTokensUnit")}
                </span>
                <span class="usage-metric-badge">
          ...
          >
            <summary class="usage-export-button">${t("usageExport")} ▾</summary>
      ...
      <div style="margin-top: 12px;">
          ...
          <div class="usage-query-actions">
            ...
            <span class="usage-query-hint">
              ${
                hasQuery
                  ? t("usageQueryHintMatch")
                      .replace("{count}", String(filteredSessions.length))
                      .replace("{total}", String(totalSessions))
                  : t("usageQueryHintInRange").replace("{total}", String(totalSessions))
              }
            </span>
          </div>
        </div>
